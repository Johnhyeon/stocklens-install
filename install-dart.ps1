...
if (-not $hasDesktopApp -and -not $hasCodeCli) {
    ...
    Warn "             설치 후 이 스크립트를 다시 실행하거나 'dartlens-setup' 을 한 번 더 돌리세요."
    Write-Host ""
}
...
for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
    ...
    if ($attempt -lt $maxAttempts) {
        Warn "      [RETRY] dartlens-setup 첫 시도 실패 ($reason). 3초 후 재시도... ($attempt/$maxAttempts)"
        Start-Sleep -Seconds 3
    }
}
if (-not $setupOk) {
    Err ""
    Err "[FAIL] dartlens-setup failed after $maxAttempts attempts ($reason). 키를 직접 다시 등록하려면:"
